...
EVAL_SCRIPT_TEMPLATE = "CUDA_LAUNCH_BLOCKING=1 NCCL_P2P_LEVEL=NVL CUDA_VISIBLE_DEVICES={cuda_devices} python {base}/ecgqa/eval.py --mode {test_mode} {extra_flags} --model {model_path} --iterations 1 --batch_size 64 --output_path {eval_output_directory}"

# Script to filter out wrong answers
FILTER_SCRIPT_TEMPLATE = "python {base}/ecgqa/filter.py --seed_data_path {seed_data_path} --input_data_path {input_data_path} --output_data_path {output_data_path}"
