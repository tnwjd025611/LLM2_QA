...
def reduce_file(file: str, output_file: str, remaining_file: str, percent: float):
    ...
    with open(remaining_file, 'w') as f:
        json.dump(excluded_data, of, indent= 4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--remaining', type=str, help='Filename to save the excluded data (use if you choice reduce)')
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--percent', type=float, help='Percentage of data to keep (use if you choose reduce mode)')
    parser.add_argument('--use_cot', type=bool, default=True, help='Whether to use COT')
    parser.add_argument('--question_type', type=str, default='ecg')
    ...
    if args.mode == 'combine':
        ...
        if len(args.files) != 1:
            raise Exception('Convert mode requires only one input file')
        convert_file(args.file[0], args.output, args.use_cot, args.question_type)
    elif args.mode == 'reduce':
        if len(args.files) != 1:
            raise Exception('Reduce mode requires only one input file')
        if not (0<=args.percent<=100):
            raise Exception('Percentage should be between 0 and 100')
        if args.remaining is None:
            raise Exception('The --remaining argument is required for the reduece mode')
        reduce_file(args.file[0], args.output, args.remaining, args.percent)